...
/*definitions for working cross-platform*/
#if defined(_WIN32) 
...
void parse_url(char *url, char **hostname, char **port, char** path) {
    ...
    /*declaring a delimiter "://" to find a protocol*/
    char *p;
    p = strstr(url, "://");
    ...
    /*finding a hostname with ":" delimiter*/
    *hostname = p;
    while (*p && *p != ':' && *p != '/' && *p != '#') 
        ++p;
...
void send_request(SOCKET s, char *hostname, char *port, char *path) {
    ...
    sprintf(buffer + strlen(buffer), "User-Agent: honpwc http_get 1.0\r\n");
